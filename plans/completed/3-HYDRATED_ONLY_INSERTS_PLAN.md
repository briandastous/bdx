...
- Upstream APIs:
  ...
  - `/twitter/tweets` (“get posts by ids”)
...
- There are no write paths that insert a `posts` row without the post payload (or equivalent) being available.
...
- **Ingest services**
  ...
  - Posts sync persists posts whose `author_id` is outside the requested target set by hydrating those author users and
...
- Add (new coverage):
  ...
  - Config-driven hydration batching:
    - `packages/config/src/env.test.ts` covers default + override behavior for `twitterapiIo.batchUsersByIdsMax` and
      `twitterapiIo.batchPostsByIdsMax` on `WorkerEnv`.
  ...
  - Hydration wrapper tests for the new batch user/post endpoints:
...
- **Posts are hydrated-only**: `posts` rows are created/updated only from an upstream post payload.
...
- [x] Define the “hydrated user” and “hydrated post” predicate in DB terms.
...
- [x] Enumerate all FK-dependent writes that currently rely on placeholder insertion.
- [x] Enumerate all config tables that persist user IDs, and classify them by whether they are **membership IDs** vs
      **seed IDs**:
...
- [x] Identify ingest/webhook metadata tables that currently require placeholder users due to FKs, and plan the FK removals
      needed to allow failure recording without domain placeholder inserts.
- [x] Confirm ingest requirement evaluation only considers successful ingests:
...
- [x] Specify failure semantics (hydration + sync):
...
- [x] Add a new `ingest_kind` value (e.g., `twitterio_api_users_by_ids`) via a DB migration.
- [x] Add a detail table for hydration runs (mirrors other ingest detail tables):
  - status/timestamps, last upstream HTTP metadata, and a join table of requested user IDs.
- [x] Extend `packages/twitterapi-io` with a first-class batch method that accepts `UserId[]` and returns `XUserData[]`
...
- [x] Add an operator entrypoint (CLI command) for:
...
- [x] Add a DB migration that makes `segment_specified_users_inputs` reference `users(id)`:
...
- [x] Update DB repository helpers:
  - `packages/db/src/repositories/assets/inputs.ts` reads/writes `user_id` (instead of `user_external_id`).
- [x] Update the config write paths (CLI/API) to hydrate missing users before inserting inputs:
  ...
  - Update any tests that call `replaceSpecifiedUsersInputs` to either pre-create users (via `upsertUserProfile`) or to
    stub hydration.
- [x] Scope decision (explicit): do not FK-fence the `segment_*_params.subject_external_id` tables in this pass.
...
### Phase 4: Implement post hydration wrapper (batch by IDs)

- [x] Add a new `ingest_kind` value (e.g., `twitterio_api_posts_by_ids`) via a DB migration.
- [x] Add a detail table for post hydration runs + requested post IDs join table.
- [x] Extend `packages/twitterapi-io` to call `/twitter/tweets`:
  - implement `tweet_ids` as a comma-separated query param (per docs).
- [x] Implement post conversion into the existing `TweetData` / `PostInput` shape.
- [x] Extend configuration:
...
- [x] Add an operator entrypoint (CLI command) to hydrate a list of post IDs (primarily for future asset types).

### Phase 5: Remove placeholder helpers and refactor all writers to hydrate first

- [x] Add a migration to allow ingest/webhook failure recording without `users` placeholder inserts:
  - drop `users` FKs from:
...
- [x] Delete `ensureUser` and `ensureUsers` from `packages/db/src/repositories/users.ts`.
- [x] Delete `upsertUserHandle` from `packages/db/src/repositories/users.ts` and update webhook ingestion to hydrate the
      target user by ID (ignore handle-only inputs).
- [x] Remove all `ensureUser` / `ensureUsers` call sites:
...
- [x] Refactor ingest services to explicitly ensure hydration before any FK-dependent writes:
...
- [x] Add targeted regression tests proving “no placeholders are created” for these flows:
