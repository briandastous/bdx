...
export async function migrateToLatestWithLock(db: Db) {
  await sql`select pg_advisory_lock(hashtext('bdx:migrations')::bigint)`.execute(
    db,
  );
  try {
    return await migrateToLatest(db);
  } finally {
    await sql`select pg_advisory_unlock(hashtext('bdx:migrations')::bigint)`.execute(
      db,
    );
  }
}
